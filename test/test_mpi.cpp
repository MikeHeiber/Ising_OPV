...
namespace MPI_Tests {
	...
	TEST_F(MPI_Test, CalculateVectorAvgTests) {
		...
		auto data_avg = MPI_calculateVectorAvg(data);
		if (procid == 0) {
	...
	TEST_F(MPI_Test, CalculateVectorSumTests) {
		...
		auto data_sum = MPI_calculateVectorSum(data);
		if (procid == 0) {
		...
		auto data_sum2 = MPI_calculateVectorSum(data2);
		if (procid == 0) {
	...
	TEST_F(MPI_Test, GatherVectorsTests) {
		...
		auto data_all = MPI_gatherVectors(data);
		if (procid == 0) {
		...
		auto data_all2 = MPI_gatherVectors(data2);
		if (procid == 0) {
